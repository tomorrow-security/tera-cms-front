			...
			<div className="flex flex-col justify-between flex-grow md:container md:mx-auto">
				...
				<div className="grid grid-cols-1 gap-8 md:grid-cols-2">
					<div className="mx-4 space-y-3 md:mx-0 md:pl-2 md:space-y-6">
						<form className="relative w-4/5 mx-auto mb-8 text-xl text-white transition-colors duration-700 rounded-t group rounded-b-xl">
							<select
							...
							>
								{jobs.map((job, index) => (
									<option
										key={index}
										value={index}
										className="transition-colors duration-700 group-hover:bg-tc-red group-focus:bg-tc-red bg-tc-blue"
									>
										{job.name}
									</option>
								))}
