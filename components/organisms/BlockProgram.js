...
const Paragraph = ({ gutterBottom, children }) => (
    <p className={`${gutterBottom && 'mb-4 md:mb-8 py-4 px-2 mx-4'}`}>{children}</p>
)
			...
			<div className="md:container md:mx-auto">
				...
				<div className="mx-2">
						<div className="mx-4 text-lg text-center">
							<Strong>Le programme Tera Campus s&apos;articule sur trois principaux axes de développement :</Strong>
						</div>
						<ul className="w-full my-12 md:my-14">
						{
							axes.map((axe, index) => (
								<li key={index} className="flex flex-row items-center justify-center w-full my-4 md:my-8">
									<div className="w-5 h-5 mr-4 transform rotate-45 bg-tc-red"> </div>
									<div className="py-1 text-2xl text-center text-white rounded-t w-52 md:w-1/3 bg-tc-blue rounded-b-xl">{axe}</div>
								</li>
							))
						}
						</ul>
						...
						<div>
							<div className="flex flex-row items-center max-w-full my-4 md:my-8">
								...
								<div className="py-1 ml-4 text-xl font-bold md:text-2xl">Les plus de Tera Campus</div>
							</div>
							<div className="grid w-11/12 mx-auto grid-programPlusRows grid-programPlusColumns gap-x-4">
								<h2 className="box-border col-start-2 row-start-1 py-2 text-xl font-medium">Le monde professionnel</h2>
								<div className= "box-border flex items-center justify-center w-12 h-12 col-start-1 row-start-3">
									<p className="w-full h-full text-center text-xl text-tc-red font-bold p-1.5 border-2 border-tc-red rounded-full">1</p>
								</div>
								...
								<h2 className="col-start-2 row-start-5 py-2 text-xl font-medium">English of course</h2>
								<div className= "box-border flex items-center justify-center w-12 h-12 col-start-1 row-start-7">
									<p className="w-full h-full text-center text-xl text-tc-red font-bold p-1.5 border-2 border-tc-red rounded-full">2</p>
								</div>
								...
								<h2 className="col-start-2 py-2 text-xl font-medium row-start-9">Les outils collaboratifs</h2>
								<div className= "box-border flex items-center justify-center w-12 h-12 col-start-1 row-start-11">
									<p className="w-full h-full text-center text-xl text-tc-red font-bold p-1.5 border-2 border-tc-red rounded-full">3</p>
								</div>
						...
						<div className="grid w-11/12 grid-cols-1 gap-4 mx-auto md:w-auto md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
								{
									cursus.map((year, index) => (
										<div key={index} className="rounded shadow">
											...
											<ul className="p-2">
												{
													year.topics.map((topic, index) => (<li key={`${index}-${topic}`} className="my-2 text-sm">{topic}</li>))
												}
											</ul>
										</div>
									))
