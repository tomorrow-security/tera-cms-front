export default function InputButton({ mutation }) {
	//TODO voir pour uniformiser les couleurs dark
	const submitButtonParams = {
		...
		default: {
			value: "Envoyer",
			color:
				"bg-tc-blue group-hover:bg-tc-red shadow group-hover:shadow-none cursor-pointer  transition-colors duration-700 group-hover:-top-20/100 transform active:translate-y-20/100",
			position: "absolute left-0 -top-15/100 z-20",
			shadow:
				"bg-tc-blue bg-opacity-50 border border-tc-blue-dark group-hover:bg-tc-red bg-opacity-50 group-hover:border-tc-red-dark shadow group-hover:shadow-none cursor-pointer  transition-colors duration-700",
			disabled: false,
		},
	}
	...
	return (
		<div className="pt-2 mx-auto w-max group">
			<div
				className={`relative z-0 px-4 font-bold text-transparent rounded-t outline-none cursor-pointer w-max rounded-b-xl ${
			...
			>
              ...
              outline-none cursor-pointer text-white font-bold rounded-t rounded-b-xl ${
								["loading", "success"].includes(mutation.status)
